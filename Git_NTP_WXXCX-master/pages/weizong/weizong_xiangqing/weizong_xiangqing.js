  ...
  let title_id = parseInt(id);
    wx.request({
      ...
      success: function (res) {
        ...
        console.log("BBB");
        console.log('LIST', magzineList.length);
      },
    })
  //console.log(test);
  return test;
  ...
  var request = function (title_id){
    wx.request({
      ...
      success: function (res) {
        ...
        console.log('magzineList',magzineList)
        that.setData({
          List: magzineList,
          ListTitle: magzineList,
          test: magzineList
...
var GetMagazineContent = function (that, id) {
  wx.request({
    ...
    success: function (res) {
      ...
      for (var i = 0; i < res.data.content.length; i++) {
        console.log("BBB");
        //console.log(res.data.content[i].list_content);
        magzineContent.push(res.data.content[i].list_content);
      }
      that.setData({
        Content: magzineContent,
      });
      app.BASEContent = magzineContent;
      console.log('magzineContent',magzineContent);
    },
  })
};
